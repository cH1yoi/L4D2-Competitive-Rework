...
public int Native_DisplayPass(Handle hPlugin, int iParams)
{
    ...
    VoteTypeToTranslation(hVoteType, VoteTranslation_Pass, szTranslation, sizeof(szTranslation));
...
bool DisplayVote(NativeVote hVote, int[] iClients, int iCountClients, int iShowTime)
{
    ...
    // Translation
    char szTranslation[TRANSLATION_LENGTH];
    VoteTypeToTranslation(hVoteType, VoteTranslation_Display, szTranslation, sizeof(szTranslation));
...
void VoteTypeToTranslation(NativeVotesType hVoteType, VoteTranslation eTranslation,  char[] szTranslation, int iLength)
{
    switch(hVoteType)
    {
        case NativeVotesType_ChgCampaign: {
            strcopy(szTranslation, iLength, eTranslation == VoteTranslation_Display ? L4D_VOTE_CHANGECAMPAIGN_START : L4D_VOTE_CHANGECAMPAIGN_PASSED);
        }

        case NativeVotesType_ChgDifficulty: {
            strcopy(szTranslation, iLength, eTranslation == VoteTranslation_Display ? L4D_VOTE_CHANGEDIFFICULTY_START : L4D_VOTE_CHANGEDIFFICULTY_PASSED);
        }

        case NativeVotesType_ReturnToLobby: {
            strcopy(szTranslation, iLength, eTranslation == VoteTranslation_Display ? L4D_VOTE_RETURNTOLOBBY_START : L4D_VOTE_RETURNTOLOBBY_PASSED);
        }

        case NativeVotesType_AlltalkOn, NativeVotesType_AlltalkOff: {
            strcopy(szTranslation, iLength, eTranslation == VoteTranslation_Display ? L4D2_VOTE_ALLTALK_START : L4D2_VOTE_ALLTALK_PASSED);
        }

        case NativeVotesType_Restart: {
            strcopy(szTranslation, iLength, eTranslation == VoteTranslation_Display ? L4D_VOTE_RESTART_START : L4D_VOTE_RESTART_PASSED);
        }

        case NativeVotesType_Kick: {
            strcopy(szTranslation, iLength, eTranslation == VoteTranslation_Display ? L4D_VOTE_KICK_START : L4D_VOTE_KICK_PASSED);
        }

        case NativeVotesType_ChgLevel: {
            strcopy(szTranslation, iLength, eTranslation == VoteTranslation_Display ? L4D_VOTE_CHANGELEVEL_START : L4D_VOTE_CHANGELEVEL_PASSED);
        }

        default: {
            strcopy(szTranslation, iLength, L4D_VOTE_CUSTOM);
        }
    }
}
