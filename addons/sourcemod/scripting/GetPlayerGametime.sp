...
public void OnPluginStart()
{
	...
	c_LPLateload			 = CreateConVar("LPLateload", "0", "If LimitPlayer=1 and the plugin is not activated properly, does it cancel the behavior of various plugins that restrict the player due to real playertime?0:disable 1:enable", FCVAR_NOTIFY, true, 0.0, true, 1.0);	   //如果LimitPlayer=1且插件未正常启动的情况下，是否取消各种因真实游戏时长而限制玩家的插件行为？0:禁用
