		...
		for (int i = 0; i < tankHud_total; ++i)
		{
			...
			tankHud.Send(client, DummyTankHudHandler, 3);
				...
				if(!IsFakeClient(client)){
						PrintHintText(client, "控制权 : %d / 2 | 控制权百分比: %d", L4D2Direct_GetTankPassedCount(), GetTankFrustration(client));
				}
			}
...
void GetMeleePrefix(int client, char[] prefix, int length)
{
	...
	{
		case WEPID_NONE: buf = "无";
		case WEPID_PISTOL: buf = (GetEntProp(secondary, Prop_Send, "m_isDualWielding") ? "双" : "单");
		case WEPID_PISTOL_MAGNUM: buf = "沙";
		case WEPID_MELEE: buf = "近";
		default: buf = "?";
	}
...
void FillSurvivorInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			FormatEx(info, sizeof(info), "->1. 生还阵营 [%d of %d]", score, GetScavengeRoundLimit());
		}
		case GAMEMODE_VERSUS:
		{
			if (bRoundLive)
			{
				FormatEx(info, sizeof(info), "->1. 生还阵营 [%d]",
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			if (bSecondHalf)
			{
				...
				FormatEx(info, sizeof(info), "> 对手时间 [%02d:%05.2f]", iMinutes, fDuration - 60 * iMinutes);
				DrawPanelText(hSpecHud, info);
			}
		}
		...
		case GAMEMODE_VERSUS:
		{
			...
			else if (bScoremod)
			{
				...
				FormatEx(info, sizeof(info), "> 实血分: %i", healthBonus);
				DrawPanelText(hSpecHud, info);
...
void FillInfectedInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			FormatEx(info, sizeof(info), "->2. 感染阵营 [%d of %d]", score, GetScavengeRoundLimit());
		}
		case GAMEMODE_VERSUS:
		{
			FormatEx(info, sizeof(info), "->2. 感染阵营 [%d]",
...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
	...
	if (bTankHUD)
	{
		FormatEx(info, sizeof(info), "%s :: Tank 面 板", sReadyCfgName);
		...
		DrawPanelText(hSpecHud, info);
		DrawPanelText(hSpecHud, " ");
		Format(info, sizeof(info), "❀拳 : %i  |  饼 : %i  |  铁 : %i", punchCount, rockCount, propCount);
		DrawPanelText(hSpecHud, info);
		Format(info, sizeof(info), "❀        总 伤 害 : %i", totalDamage);
		DrawPanelText(hSpecHud, info);
		for (int i = 0; i < len; ++i) info[i] = '_';
		DrawPanelText(hSpecHud, info);
	}
    ...
    //Draw Spawn Time
	UpdateTankUpTime();
	Format(info, sizeof(info), "❀存   活 : (%s)", Tank_UpTime);
	DrawPanelText(hSpecHud, info);

	// Draw network
	if (!IsFakeClient(tank))
	{
		FormatEx(info, sizeof(info), "❀网   络 : %ims / %.1f", RoundToNearest(GetClientAvgLatency(tank, NetFlow_Both) * 1000.0), LM_GetLerpTime(tank) * 1000.0);
	}
	else
	{
		info = "❀网   络 : AI";
...
void FillGameInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			FormatEx(info, sizeof(info), "->3. %s (回合#%i)", sReadyCfgName, GetScavengeRoundNumber());
		...
		case GAMEMODE_VERSUS:
		{
			FormatEx(info, sizeof(info), "->3. %s (回合#%d)", sReadyCfgName, 1 + view_as<int>(InSecondHalfOfRound()));
			DrawPanelText(hSpecHud, " ");
			DrawPanelText(hSpecHud, info);
