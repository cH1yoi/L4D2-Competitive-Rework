...
#if UNSCRABBLE_LOG
static const String:g_LogFile[] = "logs\\unscramble.log";
...
public OnPluginStart()
{
	...
	#endif
	LoadTranslations("l4d_team_unscramble.phrases");
	...
	ConVar cVar;
	cVar = CreateConVar("rotoblin_unscramble_notify", "1", "0=Off, 1=在解除防错位后通知玩家可以加入游戏.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_bCvarNotify = GetConVarBool(cVar);
	HookConVarChange(cVar, OnCvarChange_Notify);

	cVar = CreateConVar("rotoblin_unscramble_novotes", "1", "0=Off, 1=在防错位之前防止投票.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public Action:US_cmdh_Vote(client, const String:command[], argc)
{
	if (g_bTeamLock){
		...
		if (GetClientTeam(client) != 1)
			CPrintToChat(client, "{default}[{olive}花{default}] %T","Voting is not enabled until unscramble is completed",client);
		return Plugin_Handled;
	}
