...
// 在 Plugin Var 部分添加新变量
int g_iTankLimit;     // 每轮允许的坦克数量上限
ConVar g_cvTankCount; // Tank数量的ConVar
...
public void OnPluginStart()
{
	...
	g_cvTankCount = FindConVar("l4d_tank_count");
}
...
public void OnRoundIsLive()
{
	...
	if (g_Gamemode == GAMEMODE_VERSUS)
	{
		...
		g_iTankLimit = (g_cvTankCount != null) ? g_cvTankCount.IntValue : 1;
	}
}
...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (GetEntProp(i, Prop_Send, "m_zombieClass") == 8) // Tank
		{
			tanks[tankCount++] = i;
			hasTank = true;
		}
	...
	{
		DrawPanelText(hSpecHud, " ");
		FormatEx(info, sizeof(info), "->3. Tank (%d/%d)", tankCount, g_iTankLimit);
		DrawPanelText(hSpecHud, info);
	}
	...
	for (int i = 0; i < tankCount; i++)
	{
		...
		if (!IsFakeClient(tank))
		{
			...
			Format(info, sizeof(info), "Tank #%d : %s (%s)", i+1, name, info);
		}
		else
		{
			Format(info, sizeof(info), "Tank #%d : AI (%s)", i+1, info);
		}
		DrawPanelText(hSpecHud, info);
		...
		{
			FormatEx(info, sizeof(info), "Health  : Dead");
		}
		else
		{
