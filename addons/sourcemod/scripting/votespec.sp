...
public Action Cmd_AdminCallSpec(int client, int args)
{
	...
	if (IsCharNumeric(argument[0]))
	{
		...
		g_iTargetClient = 0;
		CPrintToChatAll("{default}[提示]：管理员 {lightgreen}%N {default}将玩家 {lightgreen}%N {default}移到旁观", client, target_client);
	}
	else
	{
...
public int VoteSpecMenuHandler(Menu menu, MenuAction action, int client, int item)
{
	if (action == MenuAction_Select)
	{
		...
		if (IsValidClient(target_client) && !IsFakeClient(target_client))
		{
			...
			{
				CPrintToChat(client, "{default}[错误]：{orange}现在无法进行新的投票");
				return 0;
			}
			if (IsBuiltinVoteInProgress())
			{
				CPrintToChat(client, "{default}[错误]：{orange}现在已经有一个投票正在进行，无法进行新的投票");
				return 0;
			}
			// 统计参与投票的人数
			int player_num = 0;
			...
			CPrintToChatAll("{default}[提示]：玩家 {lightgreen}%N {default}发起了一个将玩家 {lightgreen}%N {default}移动到旁观的投票", client, target_client);
		}
		else
		{
			g_iTargetClient = 0;
			CPrintToChat(client, "{default}[错误]：选择的玩家 {lightgreen}%N {default}不是有效的玩家", target_client);
		}
	}
	return 0;
}
	...
	for (int index = 0; index < num_items; index++)
	{
		...
		if (item_info[index][BUILTINVOTEINFO_ITEM_INDEX] == BUILTINVOTES_VOTE_YES)
		{
			if (item_info[index][BUILTINVOTEINFO_ITEM_VOTES] > num_votes * RoundToNearest(g_hVotePercent.FloatValue * 0.01))
			{
				...
				CPrintToChatAll("{default}[提示]：已将玩家 {lightgreen}%N {default}移到旁观", g_iTargetClient);
				// 重置 g_iTargetClient
				g_iTargetClient = 0;
				return 0;
			}
...
public Action evt_TeamChange(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		CPrintToChat(client, "{default}[提示]：你在 {orange}%d {default}分钟不允许加入任何团队，剩余 {orange}%d 分钟 %d 秒", g_hBanJoinSur.IntValue, RoundToNearest(g_fBanJoinTime[client] + g_hBanJoinSur.FloatValue * 60.0 - GetGameTime()) / 60, RoundToNearest(g_fBanJoinTime[client] + g_hBanJoinSur.FloatValue * 60.0 - GetGameTime()) % 60);
