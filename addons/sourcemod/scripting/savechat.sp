...
#define PLUGIN_VERSION "SaveChat_1.2.3"
...
public OnPluginStart(){
    ...
    // Logger 初始化，明确指定使用 LoggerType_NewLogFile
...
public Action Timer_PerformWho(Handle timer, int target)
{
    ...
    else
    {
        ...
        else
        {
            ...
            player.info("'%s' 为 本地管理员，拥有权限 '%s'", name, flagstring);
        }
    }
    return Plugin_Stop;
}   
...
public int SteamWorks_OnValidateClient(int ownerauthid, int authid)
{
    ...
    if(ownerauthid != authid) {
        log.info("'%N' 为家庭共享账户，主账户authid为 '[U:1:%i]' !请知悉，中间一位数:1:可能为0", client, ownerauthid);
        player.info("'%N' 为家庭共享账户，主账户authid为 '[U:1:%i]' !请知悉，中间一位数:1:可能为0", client, ownerauthid);
    }
}
...
stock int CustomFlagsToString(char[] buffer, int maxlength, int flags)
{
    char joins[6][6];
    int total;
 ...
 */
public int LogChat(client, args, bool teamchat)
{
    char msg[2048];
    char text[1024];
    char country[3];
